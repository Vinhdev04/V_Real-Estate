...
export const register = async (req, res) => {
    ...
    // Log yêu cầu đăng ký
    logger.info('Yêu cầu đăng ký tài khoản', { email, username });
    ...
    if (password.length < 6) {
        logger.warn('Đăng ký thất bại - Mật khẩu quá yếu', { email });
        return res
            .status(400)
    ...
    try {
            ...
            if (existingUser.loginType === 'google' && !existingUser.password) {
                ...
                await prisma.user.update({
                    ...
                    data: {
                        ...
                        loginType: 'email_google'
                    }
                });

                logger.success(
                    'Cập nhật mật khẩu cho tài khoản Google thành công',
                    {
                        email,
        ...
        if (existingUsername) {
            ...
            return res
                .status(400)
                .json({ message: 'Tên đăng nhập này đã tồn tại.' });
        }
        ...
        logger.success('Đăng ký tài khoản thành công', {
            email,
            userId: newUser.id,
            username
        });
        ...
        logger.error('Lỗi hệ thống khi đăng ký', err);
...
export const login = async (req, res) => {
    try {
        ...
        logger.info('Yêu cầu đăng nhập', { email });

        // Validation
        if (!email || !password) {
            logger.warn('Đăng nhập thất bại - Thiếu thông tin đăng nhập', {
                email
            });
            return res
                .status(400)
        ...
        if (!user) {
            logger.warn('Đăng nhập thất bại - Không tìm thấy người dùng', {
                email
            });
            return res
                .status(404)
        ...
        if (!user.password) {
            logger.warn(
                'Đăng nhập thất bại - Tài khoản Google chưa có mật khẩu',
                { email }
            );
            return res.status(400).json({
                message:
        ...
        if (!isPasswordCorrect) {
            logger.warn('Đăng nhập thất bại - Mật khẩu không đúng', { email });
            return res
                .status(401)
                .json({ message: 'Email hoặc mật khẩu không đúng!' });
        }
        ...
        logger.error('Lỗi hệ thống khi đăng nhập', err);
        res.status(500).json({
...
export const googleLogin = async (req, res) => {
    try {
        ...
        logger.info('Yêu cầu đăng nhập Google', { email, username, googleId });

        // Validation
        if (!email || !googleId) {
            logger.warn('Đăng nhập Google thất bại - Thiếu thông tin', {
                email
            });
            return res.status(400).json({
        ...
        if (!user) {
            ...
            logger.success('Tạo tài khoản Google mới thành công', {
                email,
                userId: user.id,
                username: uniqueUsername
            });
            ...
            user = await prisma.user.update({
                ...
                data: {
                    ...
                    loginType: user.password ? 'email_google' : 'google'
                }
            });

            logger.info('Cập nhật thông tin tài khoản Google', {
                email,
                userId: user.id
            });
        }
        ...
        logger.success('Đăng nhập Google thành công', {
            email,
            userId: user.id
        });
        ...
        logger.error('Lỗi hệ thống khi đăng nhập Google', err);
        res.status(500).json({
...
export const logout = async (req, res) => {
    const userId = req.userId; // Nếu có middleware verify token
    logger.info('Người dùng đăng xuất', { userId });
